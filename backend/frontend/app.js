...
async function renderHome() {
  try {
    ...
    const colors = ['#3b82f6','#8b5cf6','#ec4899','#f97316','#14b8a6','#84cc16','#06b6d4','#ef4444','#6366f1','#eab308'];
    let html = '<h2 class="section-title">Бүлгүүд</h2>';
    cats.forEach((c, i) => {
      const color = colors[i % colors.length];
      html += `
      <div class="cat-item ${c.locked ? 'locked' : ''}" onclick="${c.locked ? `nav('pro')` : `nav('category','${c.category_id}')`}">
        <div class="cat-icon" style="background:${color}20;color:${color}">${i + 1}</div>
        <div class="cat-info">
          <div class="cat-name">${esc(c.name)}</div>
          <div class="cat-meta">
            <span>${c.completed}/${c.questionCount}</span>
            ${c.locked ? '<span>🔒 PRO</span>' : ''}
          </div>
...
async function renderExamMenu() {
  try {
    ...
    html += `
      ...
      <div class="card">
        <p style="font-weight:600;margin-bottom:4px">Шинэ шалгалт эхлүүлэх</p>
        <p style="font-size:.82rem;color:var(--text3);margin-bottom:12px">20 асуулт · 25 минут · ≥75% тэнцэнэ</p>
        <button class="btn btn-primary btn-block" onclick="startExam()">Эхлүүлэх</button>
      </div>`;
...
function renderExamResult(result) {
  ...
  let html = `
    <div class="card" style="text-align:center;padding:24px">
      ...
      <h2 style="margin-bottom:4px">${result.passed ? 'Тэнцлээ!' : 'Тэнцсэнгүй'}</h2>
      <p style="color:var(--text3);font-size:.88rem;margin-bottom:16px">
        ${result.passed ? 'Баяр хүргэе! Та амжилттай тэнцлээ.' : `Тэнцэхэд ≥75% шаардлагатай. Та ${result.percent}% авлаа.`}
      </p>
      ${result.durationSeconds ? `<p style="font-size:.82rem;color:var(--text3)">Хугацаа: ${Math.floor(result.durationSeconds / 60)} мин ${result.durationSeconds % 60} сек</p>` : ''}
    </div>`;
...
async function renderStats() {
  try {
    ...
    let html = `
      ...
      <div class="stat-grid">
        <div class="stat-card"><div class="stat-value">${s.totalAnswered}</div><div class="stat-label">Хариулсан</div></div>
        <div class="stat-card"><div class="stat-value">${s.correctPercent}%</div><div class="stat-label">Зөв хариулт</div></div>
        <div class="stat-card"><div class="stat-value">${s.examsTaken}</div><div class="stat-label">Шалгалт өгсөн</div></div>
        <div class="stat-card"><div class="stat-value">${s.examsPassed}</div><div class="stat-label">Тэнцсэн</div></div>
        <div class="stat-card"><div class="stat-value">${s.currentStreak}</div><div class="stat-label">Дараалсан өдөр</div></div>
        <div class="stat-card"><div class="stat-value">${s.bookmarks}</div><div class="stat-label">Хадгалсан</div></div>
      </div>`;
...
async function renderProfile() {
  ...
  let html = `
    <div class="card profile-section">
      ${pic ? `<img class="profile-avatar" src="${esc(pic)}" alt="">` : ''}
      <h3>${esc(user?.name || 'Хэрэглэгч')}</h3>
      <p class="profile-email">${esc(email)}</p>
      ${user?.isPro ? '<div style="margin-top:8px"><span class="badge badge-pro">PRO</span></div>' : ''}
    </div>
    <div class="card">
      <h3 style="font-size:.92rem;margin-bottom:8px">Профайл нэр</h3>
      <p style="font-size:.78rem;color:var(--text3);margin-bottom:8px">3-20 тэмдэгт, латин үсэг/тоо/доогуур зураас</p>
      <input class="profile-name-input" id="pname" value="${esc(name)}" placeholder="profile_name" maxlength="20">
      <div id="pname-status" style="font-size:.78rem;margin-top:6px"></div>
      <button class="btn btn-primary btn-block" style="margin-top:8px" onclick="saveName()">Хадгалах</button>
    </div>`;
  if (!user?.isPro) {
    html += `
      <div class="card" style="text-align:center">
        <p style="font-weight:600;margin-bottom:8px">PRO болоод бүх боломжийг нээгээрэй</p>
        <button class="btn btn-primary" onclick="nav('pro')">PRO болох</button>
      </div>`;
  }
  html += `<button class="btn btn-danger btn-block" style="margin-top:16px" onclick="logout()">Гарах</button>`;
  $('#app').innerHTML = html;
  ...
  if (user?.isPro) {
    $('#app').innerHTML = `
      <div class="card" style="text-align:center;padding:24px">
        ...
        <h2>Та PRO хэрэглэгч!</h2>
        <p style="color:var(--text3);margin-top:8px">Бүх бүлэг, бүх боломж танд нээлттэй.</p>
        <button class="btn btn-primary" style="margin-top:16px" onclick="nav('home')">Нүүр хуудас</button>
      </div>`;
    return;
  }
  try {
